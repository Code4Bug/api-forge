...
function isHttpSendError(result: HttpSendResult): result is Extract<HttpSendResult, { ok: false }> {
...
function findApiNode(nodes: RequestDefinition[] | undefined, requestId?: string) {
...
export default function TestCenterPage() {
  ...
  const [headers, setHeaders] = useState<HttpFieldItem[]>(cloneFields(selectedRequest?.headers, 'header'))
  ...
  function resolveFields(fields: HttpFieldItem[]) {
    return fields.filter((item) => item.enabled && item.key.trim()).map((item) => ({ key: resolveValue(item.key), value: resolveValue(item.value) }))
  }
  ...
  async function sendRequest(payload: { name: string; method: HttpMethod; url: string; params: HttpFieldItem[]; headers: HttpFieldItem[]; body?: string }) {
    ...
    const response = await window.desktopApi.httpSend({
      ...
      params: payload.params.filter((item) => item.enabled && item.key.trim()).map((item) => ({ key: resolveValue(item.key), value: resolveValue(item.value), enabled: item.enabled })),
      headers: Object.fromEntries(payload.headers.filter((item) => item.enabled && item.key.trim()).map((item) => [resolveValue(item.key), resolveValue(item.value)])),
    ...
    try {
      ...
      for (const step of scenarioSteps) {
        ...
        results.push(response.ok
          ? { name: request.name, ok: true, status: response.status, durationMs: response.durationMs }
          : { name: request.name, ok: false, message: getErrorMessage(response) })
      }
  ...
  async function runLoadTest() {
    ...
    try {
      ...
      let failure = 0
      let cursor = 0
      const workers = Array.from({ length: Math.max(1, loadConcurrency) }, async () => {
            ...
            <div className="rounded border border-zinc-800 bg-zinc-950/60 p-3">
              ...
              <div className="space-y-1">
                {([
                  ...
                  <button key={key} onClick={() => setTab(key)} className={`flex h-8 w-full items-center rounded px-3 text-left text-xs ${tab === key ? 'bg-cyan-400/12 text-cyan-100' : 'text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200'}`}>{label}</button>
                ))}
              </div>
            </div>
            ...
            <section className="space-y-4">
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
                <div className="rounded border border-zinc-800 bg-[#111821] p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">基于已保存接口直接发起请求</p>
                    </div>
                    <button onClick={() => void runSingleTest()} disabled={singleLoading || !selectedRequest} className="flex h-9 items-center gap-2 rounded bg-cyan-400 px-3 text-xs font-semibold text-zinc-950 disabled:opacity-40">{singleLoading ? <Square className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}{singleLoading ? '执行中' : '开始测试'}</button>
                  </div>
                  <div className="grid gap-3 md:grid-cols-2">
                    <label className="text-xs text-zinc-400">方法<ThemedSelect className="mt-2" value={method} options={buildHttpMethodOptions()} onChange={(value) => setMethod(value as HttpMethod)} /></label>
                    <label className="text-xs text-zinc-400">URL<input value={url} onChange={(event) => setUrl(event.target.value)} className="mt-2 h-9 w-full rounded border border-zinc-700 bg-zinc-950 px-3 text-xs text-zinc-100 outline-none focus:border-cyan-400/60" /></label>
                  </div>
                  <label className="mt-3 block text-xs text-zinc-400">Body<textarea value={body} onChange={(event) => setBody(event.target.value)} className="mt-2 min-h-44 w-full rounded border border-zinc-700 bg-zinc-950 px-3 py-2 font-mono text-xs text-zinc-200 outline-none focus:border-cyan-400/60" /></label>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <div className="rounded border border-zinc-800 bg-zinc-950/70 p-3">
                      <div className="mb-2 text-xs font-medium text-zinc-300">参数</div>
                      <div className="space-y-2">
                        {params.map((item, index) => <input key={item.id} value={`${item.key}=${item.value}`} onChange={(event) => {
                          const [nextKey = '', ...rest] = event.target.value.split('=')
                          const nextValue = rest.join('=')
                          setParams((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, key: nextKey, value: nextValue } : entry))
                        }} className="h-8 w-full rounded border border-zinc-700 bg-zinc-950 px-2 text-[11px] text-zinc-300 outline-none" />)}
                      </div>
                    </div>
                    <div className="rounded border border-zinc-800 bg-zinc-950/70 p-3">
                      <div className="mb-2 text-xs font-medium text-zinc-300">请求头</div>
                      <div className="space-y-2">
                        {headers.map((item, index) => <input key={item.id} value={`${item.key}: ${item.value}`} onChange={(event) => {
                          const [nextKey = '', ...rest] = event.target.value.split(':')
                          const nextValue = rest.join(':').trimStart()
                          setHeaders((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, key: nextKey, value: nextValue } : entry))
                        }} className="h-8 w-full rounded border border-zinc-700 bg-zinc-950 px-2 text-[11px] text-zinc-300 outline-none" />)}
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="rounded border border-zinc-800 bg-[#111821] p-4">
                    <div className="mb-3 flex items-center gap-2 text-xs font-medium text-zinc-300"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" />执行结果</div>
                    {singleResult ? singleResult.ok ? <div className="space-y-3 text-xs text-zinc-300">
                      <StatusPill tone="green">{singleResult.status}</StatusPill>
                      <div>耗时：{singleResult.durationMs} ms</div>
                      <div>响应大小：{singleResult.sizeBytes} bytes</div>
                      <pre className="max-h-72 overflow-auto rounded bg-zinc-950 p-3 text-[11px] text-zinc-200">{formatBody(singleResult.body)}</pre>
                    </div> : <div className="rounded border border-rose-500/25 bg-rose-500/6 p-3 text-xs text-rose-200">{getErrorMessage(singleResult)}</div> : <div className="text-xs text-zinc-500">尚未执行</div>}
                  </div>
                  <div className="rounded border border-zinc-800 bg-[#111821] p-4">
          ...
          {tab === 'scenario' && (
            <section className="rounded border border-zinc-800 bg-[#111821] p-4">
              ...
              <div className="mt-4 flex items-center justify-between">
                <div className="text-xs text-zinc-500">支持按接口顺序编排，后续可继续加断言和变量提取。</div>
                <button onClick={() => void runScenario()} disabled={scenarioLoading || !scenarioSteps.length} className="flex h-9 items-center gap-2 rounded bg-cyan-400 px-3 text-xs font-semibold text-zinc-950 disabled:opacity-40">{scenarioLoading ? <Square className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}{scenarioLoading ? '执行中' : '运行场景'}</button>
              </div>
              <div className="mt-4 space-y-2">
                {scenarioResults.length ? scenarioResults.map((item, index) => <div key={`${item.name}-${index}`} className="flex items-center justify-between rounded bg-zinc-950/70 px-3 py-2 text-xs">
                  <span>{item.name}</span>
                  <span className={item.ok ? 'text-emerald-300' : 'text-rose-300'}>{item.ok ? `${item.status} · ${item.durationMs} ms` : item.message ?? '失败'}</span>
                </div>) : <div className="text-xs text-zinc-500">暂无结果</div>}
              </div>
            </section>
          )}
            ...
            <section className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
              <div className="rounded border border-zinc-800 bg-[#111821] p-4">
                <div className="mb-4">
                  ...
                  <p className="text-xs text-zinc-500">对指定接口做并发和次数压测</p>
                </div>
                <label className="mb-3 block text-xs text-zinc-400">并发数<input type="number" min={1} value={loadConcurrency} onChange={(event) => setLoadConcurrency(Math.max(1, Number(event.target.value) || 1))} className="mt-2 h-9 w-full rounded border border-zinc-700 bg-zinc-950 px-3 text-xs outline-none" /></label>
                <label className="mb-4 block text-xs text-zinc-400">请求次数<input type="number" min={1} value={loadIterations} onChange={(event) => setLoadIterations(Math.max(1, Number(event.target.value) || 1))} className="mt-2 h-9 w-full rounded border border-zinc-700 bg-zinc-950 px-3 text-xs outline-none" /></label>
                <button onClick={() => void runLoadTest()} disabled={loadLoading || !selectedRequest} className="flex h-9 w-full items-center justify-center gap-2 rounded bg-cyan-400 px-3 text-xs font-semibold text-zinc-950 disabled:opacity-40">{loadLoading ? <Square className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}{loadLoading ? '压测中' : '开始压测'}</button>
              </div>
              <div className="space-y-4">
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  ] as Array<[string, string | number]>).map(([label, value]) => <div key={label} className="rounded border border-zinc-800 bg-[#111821] p-4"><div className="text-[11px] text-zinc-500">{label}</div><div className="mt-1 text-lg font-semibold text-zinc-100">{value}</div></div>)}
                </div>
                <div className="rounded border border-zinc-800 bg-[#111821] p-4">
                  <div className="mb-2 flex items-center gap-2 text-xs font-medium text-zinc-300"><BarChart3 className="h-3.5 w-3.5 text-cyan-300" />结果说明</div>
                  <p className="text-xs leading-6 text-zinc-500">当前版本先提供可用的压测执行和核心指标，后续可以继续加错误分布、耗时分位图和导出报告。</p>
                </div>
              </div>
            </section>
          )}
          ...
          {tab === 'report' && (
            <section className="rounded border border-zinc-800 bg-[#111821] p-4">
              <div className="mb-4">
                ...
                <p className="text-xs text-zinc-500">查看最近执行记录和测试结果摘要</p>
              </div>
              <div className="space-y-2">
                {history.length ? history.slice(0, 12).map((item) => <div key={item.id} className="rounded border border-zinc-800 bg-zinc-950/70 px-3 py-3 text-xs text-zinc-300">{summarizeHistory(item)}</div>) : <div className="rounded border border-zinc-800 bg-zinc-950/70 px-3 py-3 text-xs text-zinc-500">暂无历史记录</div>}
              </div>
            </section>
          )}
        </main>

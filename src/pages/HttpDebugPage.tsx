...
const assertionTemplates = [
  ...
  {
    label: "响应 JSON",
    value: 'String(headers["content-type"] || "").includes("json")',
  },
] as const;
...
function findJsonPath(
  ...
  if (Array.isArray(root)) {
    for (let index = 0; index < root.length; index += 1) {
      const found = findJsonPath(root[index], selected, [
        ...path,
        `[${index}]`,
      ]);
  ...
  if (root && typeof root === "object") {
    for (const [key, value] of Object.entries(root)) {
      ...
      if (key === selected)
        return `$.${nextPath.join(".").replace(/\.\[/g, "[")}`;
      const found = findJsonPath(value, selected, nextPath);
      if (found) return found;
    }
    return undefined;
  }
  return String(root) === selected
    ? `$.${path.join(".").replace(/\.\[/g, "[")}`
    : undefined;
}
      ...
      try {
        ...
        if (content !== undefined)
          return {
            ...
            data: content,
            streamData: content,
            key: `${id ?? index}-${rawData.slice(0, 16)}`,
          };
        if (choice?.finish_reason)
          return {
            ...
            id,
            rawData,
            data: `流式响应已结束（${choice.finish_reason}）`,
            streamData: "",
            key: `${id ?? index}-finish`,
          };
        return {
...
function normalizeHttpFieldItems(
  value: unknown,
  prefix: "param" | "header",
): HttpFieldItem[] {
  ...
  return value.split(/(`[^`]+`|\*\*[^*]+\*\*)/g).map((part, index) => {
    ...
    if (part.startsWith("`") && part.endsWith("`"))
      return (
        <code
          key={index}
          className="rounded bg-zinc-800 px-1 py-0.5 font-mono text-cyan-200"
        >
          {part.slice(1, -1)}
        </code>
      );
...
function MarkdownText({ value }: { value: string }) {
  ...
  const flushCode = () => {
    ...
    blocks.push(
      <pre
        ...
        className="my-2 overflow-x-auto rounded border border-zinc-700 bg-zinc-950 p-3 text-xs leading-5 text-cyan-100"
      >
        <code>{code.join("\n")}</code>
      </pre>,
    );
  ...
  flushList();
  return (
    <div className="break-words text-sm leading-7 [overflow-wrap:anywhere]">
      {blocks}
    </div>
  );
}
...
export default function HttpDebugPage() {
  ...
  const updateProcessVariable = useWorkspaceStore(
    (state) => state.updateProcessVariable,
  );
  const captureProcessVariables = useWorkspaceStore(
    (state) => state.captureProcessVariables,
  );
  ...
  const [sseDisplayMode, setSseDisplayMode] = useState<"raw" | "stream">(
    "stream",
  );
  ...
  const [processVariableDialog, setProcessVariableDialog] =
    useState<ProcessVariable>();
  ...
  const [splitRatio, setSplitRatio] = useState(() => {
    ...
    return Number.isFinite(saved) && saved >= 0.35 && saved <= 0.65
      ? saved
      : 0.54;
  });
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const nextRatio = Math.min(
        Math.max((pointerX - rect.left) / available, Math.max(0.35, minRatio)),
        Math.min(0.65, maxRatio),
      );
  ...
  const editorVariables = useMemo(
    () => ({
      ...variables,
      ...Object.fromEntries(
        (workspace?.processVariables ?? []).map((item) => [
          item.key,
          item.currentValue ?? "待获取",
        ]),
      ),
    }),
    [variables, workspace?.processVariables],
  );
  const activeApiNode = useMemo(
    () => findApiNode(workspace?.apiTree ?? [], activeApiId),
  ...
  const activeRequest = useMemo(
    () =>
      ...
      workspace?.requests.find(
        (request) => request.name === activeApiNode?.name,
      ),
    [workspace?.requests, activeApiId, activeApiNode?.name],
  );
  ...
  const requestTabs = useMemo(() => {
    if (activeApiNode && activeApiNode.protocol !== "http")
  ...
  const orderedRequestTabs = useMemo(() => {
    ...
    return [
      ...requestTabOrder.filter((tab) => available.has(tab)),
      ...requestTabs.filter((tab) => !requestTabOrder.includes(tab)),
    ];
  }, [requestTabs, requestTabOrder]);
  const [activeRequestTab, setActiveRequestTab] = useState<
    (typeof requestTabs)[number]
  >(requestTabs[0]);
  useEffect(() => {
    setRequestTabOrder((current) => [
      ...current.filter((tab) => requestTabs.includes(tab as never)),
      ...requestTabs.filter((tab) => !current.includes(tab)),
    ]);
    const defaultTab = requestTabs.includes("Body" as never)
      ? "Body"
      : requestTabs.includes("Params" as never)
        ? "Params"
        : requestTabs[0];
  ...
  useEffect(() => {
    localStorage.setItem(
      "httpRequestTabOrder",
      JSON.stringify(requestTabOrder),
    );
  }, [requestTabOrder]);
  ...
  const activeParams = useMemo(
    () => params.filter((item) => item.enabled && item.key.trim()),
    [params],
  );
  const activeHeaders = useMemo(
    () => headers.filter((item) => item.enabled && item.key.trim()),
    [headers],
  );
  ...
  useEffect(() => {
    ...
    setParams(
      nextParams.length
        ? nextParams
        : [{ id: `${activeApiId}-param-0`, key: "", value: "", enabled: true }],
    );
    setHeaders(
      nextHeaders.length
        ? nextHeaders
    ...
    const authorization =
      nextHeaders.find((item) => item.key.toLowerCase() === "authorization")
        ?.value ?? "";
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("api-forge:save-request", handleSaveRequest);
  }, []);

  function updateParam(
    index: number,
    field: keyof HttpFieldItem,
  ...
  function addParam() {
    setParams((current) => [
      ...current,
      { id: `param-${crypto.randomUUID()}`, key: "", value: "", enabled: true },
    ]);
  }
  ...
  function saveCurrentRequest() {
    ...
    const request: RequestDefinition = {
      ...
      method,
      url,
      params: normalizeHttpFields(params, "param").map((item, index) => ({
        id: `${activeApiId}-param-${index}`,
        ...item,
      })),
      headers: normalizeHttpFields(headers, "header").map((item, index) => ({
        id: `${activeApiId}-header-${index}`,
        ...item,
      })),
      body,
      bodyType,
  ...
  function buildRequestBody() {
    if (bodyType === "form-urlencoded") {
      return new URLSearchParams(
        formFields
          .filter((item) => item.enabled && item.kind === "text")
          .map((item) => [
            item.key,
  ...
  function updateBodyType(
    nextType: NonNullable<RequestDefinition["bodyType"]>,
  ) {
    setBodyType(nextType);
    const contentType =
      nextType === "form-urlencoded"
        ? "application/x-www-form-urlencoded"
        : nextType === "multipart"
          ? "multipart/form-data"
          : nextType === "json"
    ...
    setHeaders((current) => {
      ...
      return current.map((item, itemIndex) =>
        itemIndex === index
          ? { ...item, value: contentType, enabled: true }
          : item,
      );
    });
  }
  ...
  function updateBearer(value: string) {
    ...
    setHeaders((current) => {
      ...
      return current.map((item, itemIndex) =>
        itemIndex === index ? { ...item, value: `Bearer ${value}` } : item,
      );
    });
  }
  ...
  function submitProcessVariable() {
    if (!processVariableDialog?.key.trim())
      return setProcessVariableError("请输入变量名");
    if (!processVariableDialog.jsonPath.trim().startsWith("$"))
      return setProcessVariableError("JSONPath 必须以 $ 开头");
  ...
  async function sendRequest() {
    if (!window.desktopApi?.httpSend) {
      setInputError(
        "当前为浏览器预览环境，HTTP 调试需要在 Electron 桌面端运行。",
      );
      return;
    }

    const headers = Object.fromEntries(
      activeHeaders.map((item) => [
        item.key,
        replaceEnvironmentVariables(item.value, variables),
      ]),
    );
    ...
    if (unresolvedVariable) {
      setInputError(
        `变量未解析：${unresolvedVariable}。请检查当前环境或先调用过程变量的来源接口。`,
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok === true) {
        ...
        appendRequestLog(
          `响应 ${response.status}，${response.durationMs} ms，${formatBytes(response.sizeBytes)}，${contentType}`,
          response.status < 400 ? "success" : "error",
        );
        appendRequestLog(
          `响应头 ${Object.keys(response.headers).length} 个：${
        ...
        appendRequestLog(
          `请求失败 [${response.error.code}]：${response.error.message}`,
          "error",
        );
      }
      ...
      if (assertion.trim()) {
        ...
        try {
          ...
          setAssertionResult({
            ok: false,
            message: `断言表达式无效：${error instanceof Error ? error.message : "无法执行"}`,
          });
          appendRequestLog(
            `断言表达式无效：${error instanceof Error ? error.message : "无法执行"}`,
            "error",
          );
        }
      } else {
  ...
  const responseContentType = result?.ok
    ? (result.headers["content-type"] ?? "")
    : "";
  const isJsonResponse =
    result?.ok === true &&
    /(?:application\/json|\+json)(?:\s*;|$)/i.test(responseContentType);
  const formattedResponseBody =
    isJsonResponse && responseBody
      ? (() => {
          try {
  ...
  const streamText = normalizeStreamText(
    sseEvents.map((item) => item.streamData).join(""),
  );
  const responseCookies = result?.ok
    ? (result.headers["set-cookie"] ?? "")
    : "";
  const activeResponseCopyContent =
  ...
  return (
    <div
      ...
      className={`relative grid h-full min-h-0 grid-cols-1 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] overflow-auto lg:grid-cols-[var(--http-split-ratio)_minmax(0,1fr)] lg:grid-rows-1 lg:overflow-hidden ${isResizing ? "select-none" : ""}`}
      style={{ "--http-split-ratio": `${splitRatio * 100}%` } as CSSProperties}
    >
        ...
        <div className="flex min-h-14 shrink-0 flex-wrap items-center gap-2 border-b border-zinc-800 px-4 py-2">
          ...
          <div className="relative min-w-0 basis-full flex-1 sm:basis-auto sm:min-w-[180px]">
            <VariableInput
              ...
              variables={variables}
              onChange={setUrl}
              placeholder="输入请求地址，例如 {{base_url}}/api"
              className="h-9 w-full rounded border border-zinc-700 bg-transparent px-3 text-sm text-zinc-100 caret-zinc-100 outline-none placeholder:text-zinc-600 focus:border-cyan-400/60"
            />
          </div>
          <button
            onClick={loading ? stopRequest : sendRequest}
            disabled={!loading && !available}
            className={`flex h-9 items-center gap-2 rounded px-4 text-xs font-semibold text-zinc-950 disabled:opacity-60 ${loading ? "bg-rose-400 hover:bg-rose-300" : "bg-cyan-400 hover:bg-cyan-300"}`}
          >
            {loading ? (
          ...
          <button
            ...
            title={activeApiId ? "保存当前 API" : "请先从左侧打开一个 API"}
            className="flex h-9 items-center gap-2 rounded border border-zinc-700 px-3 text-xs text-zinc-300 hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
        ...
        <div className="flex h-12 shrink-0 items-end gap-6 border-b border-zinc-800 px-4 text-sm text-zinc-500">
          {orderedRequestTabs.map((tab) => (
            <button
              ...
              draggable
              onDragStart={(event) =>
                event.dataTransfer.setData("text/request-tab", tab)
              }
              onDragOver={(event) => event.preventDefault()}
              onDrop={(event) => {
                event.preventDefault();
                moveRequestTab(
                  event.dataTransfer.getData("text/request-tab"),
                  tab,
                );
              }}
              onClick={() =>
                setActiveRequestTab(tab as (typeof requestTabs)[number])
              }
              className={`cursor-grab border-b-2 pb-3 transition-colors active:cursor-grabbing ${activeRequestTab === tab ? "border-cyan-400 text-zinc-100" : "border-transparent hover:text-zinc-300"}`}
            >
              {tab}
            </button>
          ))}
            ...
            <div>
              <div className="mb-2 flex items-center justify-between text-xs font-medium text-zinc-300">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded border border-zinc-700 px-2 py-1 text-[11px] text-zinc-300 hover:border-zinc-500 hover:text-zinc-100"
                >
              ...
              <div className="space-y-2 rounded border border-zinc-800 bg-zinc-950 p-3">
                {headers.map((item, index) => (
                  <div
                    key={item.id}
                    className="grid grid-cols-[24px_minmax(0,1fr)_minmax(0,1fr)_24px] items-center gap-2"
                  >
                    <input
                      type="checkbox"
                    ...
                    <HeaderSuggestInput
                      ...
                      placeholder="key"
                      className="h-9 w-full rounded border border-zinc-800 bg-zinc-900 px-3 text-xs text-zinc-100 outline-none focus:border-cyan-400/60"
                    />
                    <HeaderSuggestInput
                      value={item.value}
                      options={
                        item.key === "Content-Type"
                          ? headerValueOptions["Content-Type"]
            ...
            <div className="space-y-4 rounded border border-zinc-800 bg-zinc-950 p-4">
              ...
              <label className="block text-xs text-zinc-400">
                ...
                <VariableInput
                  value={bearerToken}
                  variables={variables}
                  onChange={updateBearer}
                  className="mt-2 h-9 w-full rounded border border-zinc-700 bg-zinc-900 px-3 font-mono text-xs text-zinc-100 outline-none focus:border-cyan-400/60"
                  placeholder="输入 token 或 {{token}}"
                />
              </label>
              <p className="text-[11px] text-zinc-600">
                保存后会同步更新 Headers 中的 Authorization。
              </p>
            </div>
          )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ThemedSelect
                    ...
                    options={[
                      ...
                      {
                        value: "form-urlencoded" as const,
                        label: "表单 URL 编码",
                      },
                    ...
                    onChange={(value) =>
                      updateBodyType(
                        value as NonNullable<RequestDefinition["bodyType"]>,
                      )
                    }
                  />
                </div>
              ...
              {bodyType === "form-urlencoded" || bodyType === "multipart" ? (
                <div className="space-y-2 rounded border border-zinc-800 bg-zinc-950 p-3">
                  {formFields.map((field, index) => (
                    <div
                      key={field.id}
                      className="grid grid-cols-[24px_minmax(0,1fr)_minmax(0,1fr)_110px_24px] gap-2"
                    >
                      <input
                        type="checkbox"
                        ...
                        className="h-4 w-4 shrink-0 self-center justify-self-center accent-cyan-400"
                      />
                      <VariableInput
                        value={field.key}
                        variables={variables}
                ...
                <div className="min-h-[240px] flex-1 overflow-hidden rounded border border-zinc-800 bg-[#0b0f14]">
                  <VariableEditor
                    height="100%"
                    language={bodyType === "json" ? "json" : bodyType}
                    theme={editorTheme}
            ...
            <div className="space-y-4 rounded border border-zinc-800 bg-zinc-950 p-4 text-xs">
              <div>
                  ...
                  <div className="flex gap-1">
                    {[5000, 30000, 60000].map((value) => (
                      <button
                        key={value}
                        type="button"
                        onClick={() => setTimeoutValue(value)}
                        className="rounded border border-zinc-700 px-2 py-1 text-[11px] text-zinc-400 hover:border-zinc-500 hover:text-zinc-100"
                      >
                        {value / 1000}s
                      </button>
                    ))}
                ...
                <input
                  ...
                  className="h-8 w-full rounded border border-zinc-700 bg-zinc-900 px-2 text-right text-zinc-100 outline-none focus:border-cyan-400/60"
                />
              </div>
              <label className="flex items-start justify-between gap-4 rounded border border-zinc-800 bg-zinc-950/60 p-3 text-zinc-300">
                <span>
                  ...
                  <span className="mt-1 block text-[11px] text-zinc-500">
                    开启后会自动跳转到 301/302/303/307/308 目标地址。
                  </span>
                </span>
                <input
                  type="checkbox"
              ...
              <label className="flex items-start justify-between gap-4 rounded border border-zinc-800 bg-zinc-950/60 p-3 text-zinc-300">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setValidateCertificates(event.target.checked)
                  }
                  className="mt-0.5 h-4 w-4 accent-cyan-400"
                />
              </label>
          ...
          {activeRequestTab === "Info" && (
            <div className="space-y-4 rounded border border-zinc-800 bg-zinc-950 p-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="text-zinc-600">接口名称</div>
                  <div className="mt-1 text-zinc-200">
                    {activeApiNode?.name ?? "-"}
                  </div>
                </div>
                <div>
                  <div className="text-zinc-600">协议</div>
                  <div className="mt-1 uppercase text-cyan-200">
                    {activeApiNode?.protocol ?? "-"}
                  </div>
                </div>
                <div>
                  <div className="text-zinc-600">参数数量</div>
                  <div className="mt-1 text-zinc-200">{params.length}</div>
                </div>
                <div>
                  <div className="text-zinc-600">最后修改</div>
              ...
              <label className="block text-zinc-400">
                ...
                <textarea
                  ...
                  onBlur={saveCurrentRequest}
                  className="mt-2 min-h-24 w-full resize-y rounded border border-zinc-700 bg-zinc-900 p-3 text-xs leading-5 text-zinc-100 outline-none focus:border-cyan-400/60"
                  placeholder="请输入接口用途、前置条件或其他备注"
                />
              </label>
            </div>
          )}
          {activeRequestTab === "Test" && (
            <div className="space-y-4 rounded border border-zinc-800 bg-zinc-950 p-4 text-xs">
              <div className="space-y-3 rounded border border-zinc-800 bg-zinc-950/60 p-3">
                ...
                {assertionResult && (
                  <div
                    className={`rounded border px-3 py-2 text-xs ${assertionResult.ok ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-rose-500/30 bg-rose-500/10 text-rose-300"}`}
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {assertionTemplates.map((template) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setAssertion(template.value);
                        setAssertionResult(undefined);
                      }}
                      className="rounded border border-zinc-700 px-3 py-1.5 text-[11px] text-zinc-300 hover:border-cyan-400/50 hover:text-cyan-200"
                    >
                      {template.label}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {processVariableTemplates.map((template) => (
                    <button
                      ...
                      className="rounded border border-zinc-700 px-3 py-1.5 text-[11px] text-zinc-300 hover:border-emerald-400/50 hover:text-emerald-200"
                    >
                      {template.label}
                    </button>
                  ))}
              ...
              <div className="border-t border-zinc-800 pt-3 text-[11px] leading-5 text-zinc-500">
                ...
                <p>
                  <code className="text-cyan-300">status</code> 表示状态码，
                  <code className="text-cyan-300">headers</code> 表示响应头，
                  <code className="text-cyan-300">body</code> 表示响应内容（JSON
                  会自动解析）。
                </p>
                <p>
                  示例：<code className="text-zinc-300">status === 200</code>、
                  <code className="text-zinc-300">body.code === 0</code>、
                  <code className="text-zinc-300">body.data.length &gt; 0</code>
                </p>
                <p>
                  可使用 <code className="text-cyan-300">{"{{变量名}}"}</code>{" "}
                  引用当前环境变量。
                </p>
              </div>
            </div>
      ...
      <button
        ...
        className="resize-handle http-split-resize-handle group absolute inset-y-0 left-[var(--http-split-ratio)] z-20 hidden w-3 -translate-x-1/2 items-center justify-center bg-transparent lg:flex"
        style={{ cursor: "col-resize" }}
      >
        <span
          className={`resize-line relative h-full ${isResizing ? "is-resizing" : ""}`}
        >
          <span
      ...
      <section className="relative flex min-h-0 min-w-0 flex-col bg-[#0f141b]">
        {processVariableNotice && (
          <div
            role="status"
            className="process-variable-notice absolute right-4 top-4 z-40 rounded border px-3 py-2 text-xs shadow-xl backdrop-blur"
          >
            {processVariableNotice}
          </div>
        )}
        <div className="flex h-14 items-center justify-between border-b border-zinc-800 px-4">
          <div className="flex items-center gap-2">
            {result?.ok && (
              <StatusPill tone={result.status < 400 ? "green" : "red"}>
                {result.status}
              </StatusPill>
            )}
            {loading && (
        ...
        <div className="flex h-12 shrink-0 items-end justify-between border-b border-zinc-800 px-4 text-sm text-zinc-500">
          <div className="flex items-end gap-5">
            {(["Body", "Headers", "Cookies", "日志"] as const).map((tab) => (
              <button
                ...
                className={`border-b-2 pb-3 transition-colors ${activeResponseTab === tab ? "border-cyan-400 text-zinc-100" : "border-transparent hover:text-zinc-300"}`}
              >
                {tab}
              </button>
            ))}
          ...
          <button
            onClick={() =>
              void navigator.clipboard.writeText(activeResponseCopyContent)
            }
            disabled={!activeResponseCopyContent}
            className="mb-1 rounded p-2 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 disabled:opacity-30"
            title={`复制${activeResponseTab}内容`}
          >
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-4">
          ...
          {activeResponseTab === "Body" &&
            ...
            (streamSse ? (
              <div className="flex min-h-0 min-w-0 flex-1 flex-col">
                <div className="mb-3 flex shrink-0 items-center">
                  <div className="inline-flex rounded border border-zinc-700 bg-zinc-950 p-1">
                    <button
                      onClick={() => setSseDisplayMode("stream")}
                      className={`rounded px-3 py-1.5 text-xs ${sseDisplayMode === "stream" ? "bg-cyan-400/15 text-cyan-200" : "text-zinc-500 hover:text-zinc-300"}`}
                    >
                      流式
                    </button>
                    <button
                      onClick={() => setSseDisplayMode("raw")}
                      className={`rounded px-3 py-1.5 text-xs ${sseDisplayMode === "raw" ? "bg-cyan-400/15 text-cyan-200" : "text-zinc-500 hover:text-zinc-300"}`}
                    >
                      原始 JSON
                    </button>
                  </div>
                ...
                {sseDisplayMode === "stream" ? (
                  <div className="min-h-0 flex-1 overflow-y-auto rounded border border-cyan-400/20 bg-zinc-950 p-4 text-zinc-200">
                    <MarkdownText value={streamText} />
                  </div>
                ) : (
                  <div className="min-h-0 min-w-0 flex-1 space-y-2 overflow-y-auto pr-1">
                    {sseEvents.map((item) => (
                      <div
                        key={item.key}
                        className="min-w-0 overflow-hidden rounded border border-cyan-400/20 bg-zinc-950 p-3"
                      >
                        <div className="mb-1 flex gap-3 text-[11px] text-cyan-300">
                          <span>{item.event}</span>
                          {item.id && (
                            <span className="text-zinc-600">#{item.id}</span>
                          )}
                        </div>
                        <pre className="m-0 whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-mono text-xs leading-5 text-zinc-300">
                          {item.rawData}
                        </pre>
                      </div>
                    ))}
              ...
              <pre
                className={`whitespace-pre-wrap break-words [overflow-wrap:anywhere] rounded border p-4 font-mono text-xs leading-6 ${result?.ok ? "border-zinc-800 bg-zinc-950 text-zinc-300" : "border-rose-500/30 bg-rose-500/10 text-rose-200"}`}
              >
                {responseBody}
              </pre>
            ))}
          {activeResponseTab === "Headers" && result?.ok && (
            <pre className="whitespace-pre-wrap rounded border border-zinc-800 bg-zinc-950 p-4 font-mono text-xs leading-5 text-zinc-500">
              {JSON.stringify(result.headers, null, 2)}
            </pre>
          )}
          {activeResponseTab === "Cookies" &&
            (responseCookies ? (
              <pre className="whitespace-pre-wrap break-words rounded border border-zinc-800 bg-zinc-950 p-4 font-mono text-xs leading-5 text-zinc-500">
                {responseCookies}
              </pre>
            ) : (
              <div className="rounded border border-dashed border-zinc-800 bg-zinc-950 p-5 text-xs text-zinc-600">
                当前响应未提供可解析的 Cookie。
              </div>
            ))}
          {activeResponseTab === "日志" && (
            <div className="min-h-0 flex-1 overflow-y-auto rounded border border-zinc-800 bg-zinc-950 p-3 font-mono text-xs leading-6">
              {requestLogs.length === 0 ? (
                <div className="text-zinc-600">发送请求后显示详细日志</div>
              ) : (
                <div className="space-y-1.5">
                  {requestLogs.map((item, index) => (
                    <div
                      key={`${item.time}-${index}`}
                      className="flex gap-3 break-words [overflow-wrap:anywhere]"
                    >
        ...
        {processVariableDialog && (
          <form
            onSubmit={(event) => {
              event.preventDefault();
              submitProcessVariable();
            }}
            className="absolute bottom-4 left-4 right-4 z-30 rounded border border-cyan-400/30 bg-zinc-950/95 p-3 shadow-2xl backdrop-blur"
          >
            <div className="mb-2 flex items-center justify-between text-xs font-semibold text-zinc-200">
              <span>插入过程变量</span>
              <button
                type="button"
                onClick={() => setProcessVariableDialog(undefined)}
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
              <label className="text-[11px] text-zinc-400">
                ...
                <input
                  autoFocus
                  value={processVariableDialog.key}
                  onChange={(event) =>
                    setProcessVariableDialog({
                      ...processVariableDialog,
              ...
              <label className="text-[11px] text-zinc-400">
                JSONPath
                <input
                  value={processVariableDialog.jsonPath}
                  onChange={(event) =>
                    setProcessVariableDialog({
                      ...processVariableDialog,
                  ...
                  className="mt-1 h-8 w-full rounded border border-zinc-700 bg-zinc-900 px-2 font-mono text-xs text-zinc-100"
                />
              </label>
              <label className="text-[11px] text-zinc-400">
                描述
                <input
                  value={processVariableDialog.description ?? ""}
                  onChange={(event) =>
                    setProcessVariableDialog({
                      ...processVariableDialog,
            ...
            {processVariableError && (
              <div className="mt-2 text-[11px] text-rose-300">
                {processVariableError}
              </div>
            )}
            <button className="mt-3 flex h-8 items-center justify-center rounded bg-cyan-400 px-3 text-xs font-semibold text-zinc-950">
              保存并插入
            </button>
          </form>
        )}

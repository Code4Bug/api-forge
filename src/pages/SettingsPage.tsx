    ...
    <div className="flex min-h-0 flex-1 flex-col md:flex-row">
      ...
      <section className="rounded border border-zinc-800 bg-zinc-950/40 p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="text-xs text-zinc-400"><span className="mb-1 block">动效</span><ThemedSelect value={sloganEffect} options={[{ value: 'static', label: '静态' }, { value: 'typewriter', label: '打字机' }, { value: 'shimmer', label: '流光' }, { value: 'breathe', label: '呼吸' }, { value: 'marquee', label: '横向漂移' }]} onChange={updateSloganEffect} /></label>
      ...
      <section className="rounded border border-zinc-800 bg-zinc-950/40 p-4">
        ...
        <div className={`mt-4 border-t border-zinc-800 pt-4 ${!cursorGlowEnabled ? 'pointer-events-none opacity-50' : ''}`}>
          <div className="mb-4"><div className="mb-2 text-xs font-medium text-zinc-300">纹理</div><div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,7rem),1fr))] gap-2"><button onClick={() => updateCursorGlowSetting('cursorMosaicTexture', 'grid')} className={`min-h-9 break-words rounded border px-2 py-2 text-[11px] ${cursorGlowTexture === 'grid' ? 'border-cyan-400/70 bg-cyan-400/15 text-cyan-200' : 'border-zinc-700 text-zinc-400'}`}>像素网格</button><button onClick={() => updateCursorGlowSetting('cursorMosaicTexture', 'dots')} className={`min-h-9 break-words rounded border px-2 py-2 text-[11px] ${cursorGlowTexture === 'dots' ? 'border-cyan-400/70 bg-cyan-400/15 text-cyan-200' : 'border-zinc-700 text-zinc-400'}`}>颗粒点阵</button><button onClick={() => updateCursorGlowSetting('cursorMosaicTexture', 'soft')} className={`min-h-9 break-words rounded border px-2 py-2 text-[11px] ${cursorGlowTexture === 'soft' ? 'border-cyan-400/70 bg-cyan-400/15 text-cyan-200' : 'border-zinc-700 text-zinc-400'}`}>柔和光斑</button></div></div>
          <div className="mb-4"><div className="mb-2 text-xs font-medium text-zinc-300">运动</div>{[['定点光效', [['breathe', '呼吸'], ['expand', '脉冲扩散'], ['contract', '脉冲收缩'], ['blink', '闪烁'], ['steady', '静止柔光'], ['centerExpand', '中心放射'], ['centerContract', '中心收束'], ['mirrorPulse', '镜像脉冲'], ['rotate', '旋转对称'], ['ripple', '同心波纹']]], ['动态追踪', [['drift', '轻柔漂移'], ['bounce', '弹跳移动'], ['orbit', '环绕移动'], ['slideX', '水平移动'], ['slideY', '垂直移动']]]].map(([group, effects]) => <div key={String(group)} className="mb-3 last:mb-0"><div className="mb-2 text-[11px] text-zinc-500">{group}</div><div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,7rem),1fr))] gap-2">{(effects as string[][]).map(([id, name]) => <button key={id} onClick={() => updateCursorGlowEffect(id)} className={`min-h-9 break-words rounded border px-2 py-2 text-[11px] ${cursorGlowEffect === id ? 'border-cyan-400/70 bg-cyan-400/15 text-cyan-200' : 'border-zinc-700 text-zinc-400'}`}>{name}</button>)}</div></div>)}</div>
          <div><div className="mb-2 text-xs font-medium text-zinc-300">色彩</div><div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,7rem),1fr))] gap-2">{[['theme', '跟随主题'], ['cyan', '青蓝'], ['amber', '琥珀'], ['spectrum', '彩虹']].map(([id, name]) => <button key={id} onClick={() => updateCursorGlowSetting('cursorMosaicColor', id)} className={`min-h-9 break-words rounded border px-2 py-2 text-[11px] ${cursorGlowColor === id ? 'border-cyan-400/70 bg-cyan-400/15 text-cyan-200' : 'border-zinc-700 text-zinc-400'}`}>{name}</button>)}</div></div>
        </div>
      </section>
      </>}
      <ConfirmModal
      ...
      <section className="rounded border border-zinc-800 bg-zinc-950/40 p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs text-zinc-400"><span className="mb-1 block">服务商</span><ThemedSelect value={modelConfig.provider} disabled={!modelConfig.id} options={modelProviders.map((provider) => ({ value: provider, label: provider }))} onChange={selectLargeProvider} /></label>
      ...
      <section className="rounded border border-zinc-800 bg-zinc-950/40 p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs text-zinc-400"><span className="mb-1 block">服务商</span><ThemedSelect value={lightModelConfig.provider} disabled={!lightModelConfig.id} options={modelProviders.map((provider) => ({ value: provider, label: provider }))} onChange={selectLightProvider} /></label>
      ...
      {activeCategory === 'workspace' && <section className="rounded border border-zinc-800 bg-zinc-950/40 p-4">
        ...
        <label className="mt-4 flex items-center justify-between gap-4 text-xs text-zinc-400">保存间隔<ThemedSelect className="w-36" disabled={!autoSaveEnabled} value={autoSaveInterval} options={[{ value: 30, label: '每 30 秒' }, { value: 60, label: '每 1 分钟' }, { value: 300, label: '每 5 分钟' }, { value: 900, label: '每 15 分钟' }]} onChange={(value) => setAutoSaveSettings(autoSaveEnabled, value)} /></label>
      </section>}

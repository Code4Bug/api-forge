...
import { AlertCircle, Braces, Check, Database, Download, Edit3, EyeOff, Plus, Save, Trash2, Upload, X } from 'lucide-react'
...
export default function EnvironmentPage() {
  ...
  return <div className="flex h-full flex-col overflow-hidden bg-[var(--app-bg)]">
    ...
    {processDialog && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><form onSubmit={(event) => { event.preventDefault(); submitProcessVariable() }} className="w-full max-w-md rounded border border-zinc-700 bg-zinc-950 p-4"><div className="mb-3 flex items-center justify-between text-sm font-semibold">{processVariables.some((item) => item.id === processDialog.id) ? '编辑过程变量' : '新增过程变量'}<button type="button" onClick={() => { setProcessDialog(undefined); setDialogError('') }}><X className="h-4 w-4" /></button></div><label className="block text-xs text-zinc-400">变量名<input autoFocus value={processDialog.key} onChange={(event) => setProcessDialog({ ...processDialog, key: event.target.value })} className="mt-1 h-9 w-full rounded border border-zinc-700 bg-zinc-900 px-3 font-mono text-xs" placeholder="access_token" /></label><label className="mt-3 block text-xs text-zinc-400">来源接口<select value={processDialog.sourceRequestId} onChange={(event) => setProcessDialog({ ...processDialog, sourceRequestId: event.target.value })} className="mt-1 h-9 w-full rounded border border-zinc-700 bg-zinc-900 px-3 text-xs"><option value="">请选择 HTTP 接口</option>{httpRequests.map((request) => <option key={request.id} value={request.id}>{request.name}</option>)}</select></label><label className="mt-3 block text-xs text-zinc-400">响应 JSONPath<input value={processDialog.jsonPath} onChange={(event) => setProcessDialog({ ...processDialog, jsonPath: event.target.value })} className="mt-1 h-9 w-full rounded border border-zinc-700 bg-zinc-900 px-3 font-mono text-xs" placeholder="$.data.accessToken" /></label><label className="mt-3 block text-xs text-zinc-400">描述<input value={processDialog.description ?? ''} onChange={(event) => setProcessDialog({ ...processDialog, description: event.target.value })} className="mt-1 h-9 w-full rounded border border-zinc-700 bg-zinc-900 px-3 text-xs" /></label>{dialogError && <div className="mt-3 text-xs text-rose-300">{dialogError}</div>}<button className="mt-4 flex h-9 w-full items-center justify-center gap-2 rounded bg-cyan-400 text-xs font-semibold text-zinc-950"><Check className="h-3.5 w-3.5" />保存过程变量</button></form></div>}
  </div>
}

...
export function getWorkspaceVariables(
  ...
  const processVariables =
    workspace?.processVariables?.filter(
  ...
  requests: [
    {
      ...
      params: [
        ...
        { id: "size", key: "size", value: "20", enabled: true },
      ],
      headers: [
        {
          id: "content-type",
...
function normalizeModelPreferences(
  ...
  const largeModels = (
    preferences.largeModels?.length
      ? preferences.largeModels
      : preferences.largeModel
        ? [preferences.largeModel]
        : []
  ).map((item, index) => ({
    ...item,
    ...
    maxContextTokens: Number.isFinite(item.maxContextTokens)
      ? item.maxContextTokens
      : 128000,
    thinkingEnabled: item.thinkingEnabled === true,
  }));
  const lightModels = (
    preferences.lightModels?.length
      ? preferences.lightModels
      : preferences.lightModel
        ? [preferences.lightModel]
        : []
  ).map((item, index) => ({
    ...item,
  ...
  const activeLargeModelId = largeModels.some(
    (item) => item.id === preferences.activeLargeModelId,
  )
    ? preferences.activeLargeModelId
    : preferences.largeModel?.enabled
      ? largeModels[0]?.id
      : undefined;
  const activeLightModelId = lightModels.some(
    (item) => item.id === preferences.activeLightModelId,
  )
    ? preferences.activeLightModelId
    : preferences.lightModel?.enabled
      ? lightModels[0]?.id
      : undefined;
  return {
    ...preferences,
...
function saveHistory(
  ...
  saveQueue = saveQueue
    .catch(() => undefined)
    .then(() => desktopApi.saveHistory(history))
...
function updateTree(
  ...
  return nodes.map((node) =>
    updater({
      ...node,
      children: node.children ? updateTree(node.children, updater) : undefined,
    }),
  );
}
    ...
    node.id === parentId
      ...
      : {
          ...
          children: node.children
            ? appendTree(node.children, parentId, child)
            : node.children,
        },
  );
}
  ...
  return nodes.map((node) =>
    node.id === parentId
      ...
      : {
          ...node,
          children: node.children
            ? insertTree(node.children, parentId, child, index)
            : node.children,
        },
  );
}
...
function removeTree(nodes: ApiTreeNode[], nodeId: string): ApiTreeNode[] {
  return nodes
    ...
    .map((node) => ({
      ...node,
      children: node.children
        ? removeTree(node.children, nodeId)
        : node.children,
    }));
}
...
function treeContains(nodes: ApiTreeNode[], nodeId?: string): boolean {
  ...
  return nodes.some(
    (node) =>
      node.id === nodeId ||
      (node.children ? treeContains(node.children, nodeId) : false),
  );
}
...
export const useWorkspaceStore = create<WorkspaceState>((set, get) => ({
  ...
  loadWorkspace: async () => {
    let workspace = window.desktopApi
      ? await window.desktopApi.loadWorkspace()
      : fallbackWorkspace;
    const originalPreferences = workspace.preferences;
    const normalizedPreferences =
      normalizeModelPreferences(originalPreferences);
    workspace = {
      ...workspace,
    ...
    if (
      ...
      persistWorkspace(workspace, set);
  },
  setActiveProtocol: (protocol) => {
    ...
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, activeProtocol: protocol },
    };
  ...
  deleteEnvironment: (environmentId) => {
    ...
    const environments = workspace.environments.filter(
    ...
    const nextWorkspace = {
      ...
      environments: workspace.environments.map((env) =>
        env.id !== environmentId
          ...
          : {
              ...env,
              variables: env.variables.some((item) => item.id === variable.id)
                ? env.variables.map((item) =>
                    item.id === variable.id
                      ? { ...variable, key: variable.key.trim() }
                      : item,
                  )
                : [...env.variables, { ...variable, key: variable.key.trim() }],
            },
      ),
    };
    set({ workspace: nextWorkspace });
    ...
    const nextWorkspace = {
      ...
      environments: workspace.environments.map((env) =>
        env.id === environmentId
          ? {
              ...env,
              variables: env.variables.filter((item) => item.id !== variableId),
            }
          : env,
      ),
    };
  ...
  updateProcessVariable: (variable) => {
    ...
    const sourceChanged =
      previous &&
      (previous.sourceRequestId !== variable.sourceRequestId ||
        previous.jsonPath !== variable.jsonPath.trim());
    const normalized = {
      ...variable,
  ...
  deleteProcessVariable: (variableId) => {
    ...
    const nextWorkspace = {
      ...workspace,
      processVariables: (workspace.processVariables ?? []).filter(
  ...
  captureProcessVariables: (sourceRequestId, responseBody) => {
    ...
    if (
      !workspace ||
      !(workspace.processVariables ?? []).some(
        (item) => item.sourceRequestId === sourceRequestId,
      )
    )
      return;
    ...
    try {
      ...
      const processVariables = (workspace.processVariables ?? []).map((item) =>
        item.sourceRequestId === sourceRequestId
          ? {
              ...item,
              currentValue: undefined,
              lastError: "接口响应不是有效的 JSON",
              updatedAt: new Date().toISOString(),
            }
          : item,
      );
    ...
    const processVariables = (workspace.processVariables ?? []).map((item) => {
      ...
      return {
        ...item,
        currentValue: stringifyProcessVariableValue(result.value),
        lastError: undefined,
        updatedAt,
      };
    });
  ...
  setOpenApiIds: (apiIds) => {
    ...
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, openApiIds: apiIds },
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  },
  ...
  addHistory: (item) => {
    ...
    const nextWorkspace = {
      ...workspace,
      history: [item, ...workspace.history].slice(0, 200),
    };
    set({ workspace: nextWorkspace });
  ...
  createFolder: (parentId, name = "新建目录") => {
    ...
    const nextWorkspace = {
      ...workspace,
      apiTree: appendTree(workspace.apiTree, parentId, {
        id,
        type: "folder",
        name: name.trim(),
        parentId,
  ...
  moveApi: (apiId, parentId, index) => {
    ...
    const target = parentId
      ? findTreeNode(workspace.apiTree, parentId)
      : undefined;
    if (
      !api ||
      api.type !== "api" ||
      ...
      return;
    const nextApiTree = insertTree(
      removeTree(workspace.apiTree, apiId),
      parentId,
      { ...api, parentId },
      index,
    );
  ...
  renameNode: (nodeId, name) => {
    ...
    const nextWorkspace = {
      ...
      apiTree: updateTree(workspace.apiTree, (node) =>
        node.id === nodeId ? { ...node, name: name.trim() } : node,
      ),
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  ...
  updateLargeModelConfig: (config) => {
    ...
    const largeModels = current.some((item) => item.id === config.id)
      ? current.map((item) => (item.id === config.id ? normalizedConfig : item))
      : [...current, normalizedConfig];
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, largeModels },
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  },
  updateLightModelConfig: (config) => {
    ...
    const lightModels = current.some((item) => item.id === config.id)
      ? current.map((item) => (item.id === config.id ? normalizedConfig : item))
      : [...current, normalizedConfig];
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, lightModels },
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  },
  deleteLargeModelConfig: (configId) => {
    const { workspace } = get();
    if (!workspace) return;
    const largeModels = (workspace.preferences.largeModels ?? []).filter(
      (item) => item.id !== configId,
    );
    const activeLargeModelId =
      workspace.preferences.activeLargeModelId === configId
        ? undefined
  ...
  deleteLightModelConfig: (configId) => {
    const { workspace } = get();
    if (!workspace) return;
    const lightModels = (workspace.preferences.lightModels ?? []).filter(
      (item) => item.id !== configId,
    );
    const activeLightModelId =
      workspace.preferences.activeLightModelId === configId
        ? undefined
  ...
  activateLargeModelConfig: (configId) => {
    const { workspace } = get();
    if (
      !workspace?.preferences.largeModels?.some((item) => item.id === configId)
    )
      return;
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, activeLargeModelId: configId },
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  },
  activateLightModelConfig: (configId) => {
    const { workspace } = get();
    if (
      !workspace?.preferences.lightModels?.some((item) => item.id === configId)
    )
      return;
    const nextWorkspace = {
      ...workspace,
      preferences: { ...workspace.preferences, activeLightModelId: configId },
    };
    set({ workspace: nextWorkspace });
    saveWorkspace(nextWorkspace, set);
  },

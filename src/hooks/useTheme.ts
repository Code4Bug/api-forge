...
function readSavedCustomThemes(): SavedCustomTheme[] {
  try {
    ...
    if (stored !== null) {
      ...
      return Array.isArray(saved) ? saved.filter((item) => item?.id && item?.name && item?.config).map((item) => {
        const config = normalizeThemeConfig(item.config)
        return { ...item, mode: item.mode === 'light' || item.mode === 'dark' ? item.mode : getThemeMode(config), config }
      }) : []
    }
  } catch {
    // 继续尝试迁移旧版单主题配置。
  ...
  try {
    const legacy = JSON.parse(localStorage.getItem('customTheme') ?? '') as Partial<ThemeConfig>
    if (!legacy || typeof legacy !== 'object') return []
    const config = normalizeThemeConfig(legacy)
    return [{ id: 'legacy-custom-theme', name: '自定义主题 1', mode: getThemeMode(config), config, updatedAt: new Date().toISOString() }]
  } catch {
    return []
  }
}

function activeCustomTheme(themes = readSavedCustomThemes()): SavedCustomTheme | undefined {
...
function applyTheme(theme: Theme) {
  ...
  const mode = actualTheme === 'custom' ? activeCustomTheme()?.mode ?? getThemeMode(config) : getThemeMode(config)
  ...
  root.classList.add(lightTheme ? 'light' : actualTheme === 'dim' ? 'dim' : 'dark')
}
...
export function useTheme() {
  ...
  const [savedCustomThemes, setSavedCustomThemes] = useState<SavedCustomTheme[]>(readSavedCustomThemes)
  const [activeCustomThemeId, setActiveCustomThemeId] = useState(() => activeCustomTheme()?.id)
  const [customThemeMode, setCustomThemeMode] = useState<ThemeMode>(() => activeCustomTheme()?.mode ?? getThemeMode(readCustomTheme()))
  ...
  function saveCustomTheme(config: ThemeConfig, name?: string, mode = getThemeMode(config)) {
    const saved = readSavedCustomThemes()
  ...
  const actualTheme = theme === 'system' ? systemTheme() : theme
  const themeMode = actualTheme === 'custom' ? customThemeMode : getThemeMode(themePresets[actualTheme as keyof typeof themePresets] ?? themePresets.dark)
  const isDark = themeMode === 'dark'
  const monacoTheme = themeMode === 'light' ? 'light' : 'vs-dark'
  return { theme, themeMode, monacoTheme, setTheme, customTheme, savedCustomThemes, activeCustomThemeId, saveCustomTheme, activateCustomTheme, deleteCustomTheme, isDark }
}

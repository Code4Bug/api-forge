...
export function extractJsonPath(root: unknown, path: string): JsonPathResult {
  ...
  if (!normalizedPath.startsWith('$')) return { ok: false, error: 'JSONPath 必须以 $ 开头' }
  if (normalizedPath === '$') return { ok: true, value: root }

  const tokenPattern = /(?:\.([A-Za-z_$][\w$-]*)|\[(\d+)\]|\["([^"]+)"\]|\['([^']+)'\])/gy
  ...
  while (tokenPattern.lastIndex < normalizedPath.length) {
    ...
    if (!match || match.index !== start) return { ok: false, error: `不支持的 JSONPath：${normalizedPath}` }

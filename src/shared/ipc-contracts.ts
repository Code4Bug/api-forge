...
export function getActiveLargeModel(
  preferences?: UserPreferences,
): LargeModelConfig | undefined {
  if (!preferences) return undefined;
  return (
    preferences.largeModels?.find(
      (item) => item.id === preferences.activeLargeModelId,
    ) ?? (preferences.largeModel?.enabled ? preferences.largeModel : undefined)
  );
}

export function getActiveLightModel(
  preferences?: UserPreferences,
): LightModelConfig | undefined {
  if (!preferences) return undefined;
  return (
    preferences.lightModels?.find(
...
export interface DesktopApi {
  ...
  bashExec: (request: BashExecRequest) => Promise<BashExecResult>;
  onSocketEvent?: (
    listener: (payload: {
      connectionId: string;

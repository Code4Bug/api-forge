...
export function VariableEditor({ variables, onInsertProcessVariable, ...props }: Props) {
  const variablesRef = useRef(variables)
  variablesRef.current = variables

  const onMount: OnMount = (editor, monaco) => {
    const disposable = monaco.languages.registerCompletionItemProvider(props.language ?? 'plaintext', {
      triggerCharacters: ['{'],
    ...
    const processVariableAction = onInsertProcessVariable ? editor.addAction({
      ...
      run: (currentEditor) => {
        ...
        if (!selection) return
        onInsertProcessVariable(currentEditor.getModel()?.getValueInRange(selection) ?? '')
      },
    }) : undefined
    editor.onDidDispose(() => {
      disposable.dispose()

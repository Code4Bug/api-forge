...
export function VariableInput({ value, variables, onChange, multiline = false, className = '', ...props }: Props) {
  ...
  const preview = <div ref={previewRef} aria-hidden="true" className={`pointer-events-none absolute inset-0 z-0 overflow-hidden !border-transparent !bg-transparent ${multiline ? 'whitespace-pre-wrap break-words' : 'flex items-center whitespace-pre'} ${className}`}><span>{parts.map((part, partIndex) => part.match(/^\{\{[^{}]+\}\}$/) ? <span key={`${part}-${partIndex}`} style={{ color: 'var(--app-accent)' }}>{part}</span> : <span key={`${part}-${partIndex}`}>{part}</span>)}</span></div>

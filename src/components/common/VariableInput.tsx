...
export function VariableInput({
  ...
  const previewClassName = className
    .split(/\s+/)
    .filter((token) =>
      /^(font-|text-(?:xs|sm|base|lg|\[)|leading-|tracking-|p[trblxy]?-[0-9])/.test(
        token,
      ),
    )
    .join(" ");
  const preview = (
    <div
      ref={previewRef}
      aria-hidden="true"
      className={`variable-input-preview pointer-events-none absolute inset-0 z-0 overflow-hidden ${multiline ? "variable-input-preview-multiline whitespace-pre-wrap break-words" : "whitespace-pre"} ${previewClassName}`}
    >
      <span>
  ...
  const field = multiline ? (
    <textarea
      ...
      onFocus={() => setOpen(value.match(/\{\{[^{}]*$/) !== null)}
    ...
    <div className="relative min-w-0">
      ...
      {open && suggestions.length > 0 && (
        <div className="absolute left-0 top-full z-40 mt-1 max-h-44 w-full overflow-auto rounded border border-zinc-700 bg-[#111821] py-1 shadow-xl">
          {suggestions.map((key, suggestionIndex) => (
            <button
              key={key}
              type="button"

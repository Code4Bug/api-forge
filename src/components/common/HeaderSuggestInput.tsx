import { useMemo, useState, type InputHTMLAttributes, type KeyboardEvent } from 'react'

type HeaderSuggestInputProps = Omit<InputHTMLAttributes<HTMLInputElement>, 'value' | 'onChange'> & {
  value: string
  options: readonly string[]
  onChange: (value: string) => void
}

export function HeaderSuggestInput({ value, options, onChange, className = '', onFocus, onBlur, onKeyDown, ...props }: HeaderSuggestInputProps) {
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    if (open && suggestions.length > 0) {
      if (event.key === 'ArrowDown') { event.preventDefault(); setIndex((current) => (current + 1) % suggestions.length); return }
      if (event.key === 'ArrowUp') { event.preventDefault(); setIndex((current) => (current - 1 + suggestions.length) % suggestions.length); return }
  ...
  return (
    <div className="relative min-w-0">
      <input
        ...
        onFocus={(event) => { setOpen(Boolean(suggestions.length)); onFocus?.(event) }}
